...
完整命中依精確度分為**三個互斥等級**；另外保留「部分符合」與「資料未提供」兩個無法排除區：
...
```bash
npm test                          # 110 項：搜尋語意／正規化／管線 fail-closed／回歸 fixtures／靜態契約
...
規格在 `.ai-review/plan.md`；同目錄另有兩輪規格覆審（`plan-review*.md`）、
一輪程式碼覆審（`codex-review.md`）與各自的判定，以及 `evidence/` 的人工驗收留檔。
工程慣例見 `CLAUDE.md`。

Service Worker 的行為（快取上限是否真的成立、部署後首次重載拿到哪一版、離線時畫面說了什麼）
**靜態測試驗不到**，改動 `sw.js` 後請重跑 `.ai-review/evidence/e11-2026-08-13.md` 那六項。
